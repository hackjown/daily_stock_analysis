...
启用 `LLM_CHANNELS` 后，运行时只读取对应渠道下的 `LLM_<CHANNEL>_*` 配置，不会再读取同一服务商的 legacy key。比如 `LLM_CHANNELS=aihubmix` 时需要配置 `LLM_AIHUBMIX_API_KEY` / `LLM_AIHUBMIX_MODELS`；如果只填了 `AIHUBMIX_KEY`，请把 key 移到渠道变量，或删除 `LLM_CHANNELS` 回到极简 legacy 模式。DeepSeek、OpenAI、Gemini、Anthropic 同理。
...
| `missing_channel_api_key` / `LLM_<CHANNEL>_API_KEY` | 启用了 `LLM_CHANNELS`，但对应渠道没有配置 key；常见于只填了 `AIHUBMIX_KEY` / `DEEPSEEK_API_KEY` 等 legacy key。 | 改填 `LLM_<CHANNEL>_API_KEY` 或 `LLM_<CHANNEL>_API_KEYS`；如果只想用极简模式，则删除 `LLM_CHANNELS`。GitHub Actions 中密钥字段必须放 Repository Secrets。 |
| `missing_channel_models` / `LLM_<CHANNEL>_MODELS` | 启用了渠道，但没有为该渠道声明模型列表。 | 配置 `LLM_<CHANNEL>_MODELS`，例如 `LLM_AIHUBMIX_MODELS=gpt-5.5`；或在 Web 设置页重新选择/保存模型。 |
